...
**Özyeğin University** (`scrapers/ozyegin.py`) is higher-uncertainty than every other adapter in this repo. Every other adapter's fixture was built (or later rewritten) from real, directly-inspected raw HTML — Özyeğin's fixture is a **best-effort reconstruction** from markdown-extracted text only, because the real page's DOM structure (Elementor page builder, no `<table>` anywhere) could not be directly confirmed at the time it was written. The fixture assumes program name and fee share a single concatenated text block (`"Computer Engineering$25,000"`); this has not been confirmed against the live page's actual markup.

This matters because of the **Üsküdar off-by-one incident** (see git history / `data/review_*.md` from 2026-07): an adapter that passed its own tests still silently recorded the wrong fee for every single program, because the fixture's structure didn't match the live page closely enough — and the tests, built against that same wrong fixture, couldn't catch it. A parser that raises loudly on a genuine structure mismatch is a floor, not a substitute for checking the real page. Before trusting any proposal this adapter generates in `fee_change_proposals`, cross-check it against a real run (e.g. confirm Computer Engineering comes out to $25,000, matching the known figure) the same way that incident was eventually caught — don't assume passing fixture tests mean the live scrape is correct.
